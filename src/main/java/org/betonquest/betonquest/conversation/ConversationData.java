...
public class ConversationData {
    ...
    public ConversationData(final BetonQuestLogger log, final QuestPackageManager packManager,
                            ...
                            final ConfigurationSection convSection,
    ...
    public void checkExternalPointers() throws QuestException {
        for (final CrossConversationReference externalPointer : externalPointers) {
            ...
            final QuestPackage targetPack = resolvedPointer.conversationData().getPack();
            final String targetConvName = resolvedPointer.conversationData().publicData.conversationID.get();
            ...
            if (!conv.hasOption(resolvedPointer)) {
                log.warn(conv.getPack(), "External pointer in '" + externalPointer.sourcePack() + "' package, '" + externalPointer.sourceConv() + "' conversation, "
     ...
     */
    private CrossConversationReference resolvePointer(
            @Nullable final String currentOptionName, final OptionType optionType,
            final String option) throws QuestException {
        final ConversationOptionResolver resolver = new ConversationOptionResolver(packManager, featureApi, getPack(), publicData.conversationID.get(), optionType, option);
        return new CrossConversationReference(getPack(), publicData.conversationID.get(), currentOptionName, resolver);
    }
    ...
    public boolean isReady(final Profile profile) throws QuestException {
        for (final String option : getStartingOptions()) {
            ...
            if (option.contains(".")) {
                final ResolvedOption result = new ConversationOptionResolver(packManager, featureApi, getPack(), publicData.conversationID.get(), NPC, option).resolve();
        ...
        private List<String> getPointers(@Nullable final Profile profile, final List<String> optionPath) {
            ...
            if (profile != null) {
                for (final String extend : extendLinks) {
                    ...
                    try {
                        resolvedExtend = new ConversationOptionResolver(packManager, featureApi, getPack(), publicData.conversationID.get(), type, extend).resolve();
                    } catch (final QuestException e) {
