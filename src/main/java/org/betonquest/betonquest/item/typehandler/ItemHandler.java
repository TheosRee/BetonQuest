...
public interface ItemHandler<T, G> {
     ...
     */
    void populate(T meta) throws QuestException;
     ...
     * @throws QuestException when there is an exception while resolving profile specific data
     */
    default void populate(final T specific, @Nullable final Profile profile) throws QuestException {
        populate(specific);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    default void rawPopulate(final G generic, @Nullable final Profile profile) throws QuestException {
        if (clazz().isInstance(generic)) {
