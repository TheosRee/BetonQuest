...
public class ItemProcessor extends TypedQuestProcessor<ItemID, QuestItem> {
    ...
    @Override
    protected ItemID getIdentifier(final QuestPackage pack, final String identifier) throws ObjectNotFoundException {
     ...
     * @throws ObjectNotFoundException if there exists no QuestItem with that id
     */
    public QuestItem getItem(final ItemID itemId) throws ObjectNotFoundException {
        final QuestItem item = values.get(itemId);
        if (item == null) {
            throw new ObjectNotFoundException("Tried to load item '" + itemId
    ...
    public QuestItem generate(final String instruction) throws QuestException {
        final Instruction parsed = new Instruction(log, dummy, null, instruction);
