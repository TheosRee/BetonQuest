 ...
 */
public class MMInteractCatcher extends NpcInteractCatcher<ActiveMob> {
    /**
     * Initializes the interact catcher.
     *
     * @param npcTypeRegistry the registry to identify the clicked Npc
     */
    public MMInteractCatcher(final NpcTypeRegistry npcTypeRegistry) {
    ...
    public void onRight(final MythicMobInteractEvent event) {
        if (interactLogic(event.getPlayer(), new MythicMobNpcAdapter(event.getActiveMob()), Interaction.RIGHT,
