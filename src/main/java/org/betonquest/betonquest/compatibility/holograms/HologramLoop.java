...
public abstract class HologramLoop extends SectionProcessor<HologramLoop.HologramID, HologramWrapper> {
    ...
    protected HologramWrapper loadSection(final QuestPackage pack, final ConfigurationSection section) throws QuestException {
        ...
        final Variable<Number> maxRange = new Variable<>(variableProcessor, pack, section.getString("max_range", "0"), Argument.NUMBER);
    ...
    private TopLine parseTopLine(final QuestPackage pack, final String line) throws QuestException {
        ...
        final Component colorPlace = getColorCodes(validator.group(4));
        final Component colorName = getColorCodes(validator.group(5));
        final Component colorDash = getColorCodes(validator.group(6));
        final Component colorScore = getColorCodes(validator.group(7));
    ...
    private Component getColorCodes(@Nullable final String color) throws QuestException {
        if (color != null) {
            return textParser.parse(color);
        }
