package org.betonquest.betonquest.atlas.simplenpc;

import com.ags.simplenpcs.api.NPCShowEvent;
import org.betonquest.betonquest.api.quest.npc.DefaultNpcHider;
import org.bukkit.event.EventHandler;
import org.bukkit.event.EventPriority;
import org.bukkit.event.Listener;

/**
 * Prevents respawning of hidden Npcs.
 */
public class SimpleHider implements Listener {

    /**
     * Hider to check current visibility.
     */
    private final DefaultNpcHider npcHider;

    /**
     * Create a new Fancy Hider to force Npc hiding.
     *
     * @param npcHider the npc hider to check if Npc is hidden
     */
    public SimpleHider(final DefaultNpcHider npcHider) {
        this.npcHider = npcHider;
    }

    /**
     * Cancels npc sending to player if the hide conditions are met.
     *
     * @param event the spawn event to listen
     */
    @EventHandler(priority = EventPriority.LOWEST, ignoreCancelled = true)
    public void onSpawn(final NPCShowEvent event) {
        if (npcHider.isHidden(new SimpleAdapter(event.getNpc()), event.getPlayer())) {
            event.getNpc().hide(event.getPlayer());
        }
    }
}
